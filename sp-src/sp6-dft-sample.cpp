...
void create_complex_dft_image(const cv::Mat &in, cv::Mat &out) {
     ...
     */
    copyMakeBorder(in, real_image, 0, cv::getOptimalDFTSize(in.rows) - in.rows,
                   0, cv::getOptimalDFTSize(in.cols) - in.cols,
                   cv::BORDER_CONSTANT, cv::Scalar::all(0));

    // 0で埋めた虚部とマージして複素画像にする。
    cv::Mat planes[] = {cv::Mat_<float>(real_image),
                        cv::Mat::zeros(real_image.size(), CV_32F)};
    cv::merge(planes, 2, out);
}

// DFTの結果である複素画像を表示用の強度画像に変換する。
void create_fourier_magnitude_image_from_complex(const cv::Mat &in,
                                                 cv::Mat &out) {
    /**
    ...
    cv::Mat q0(out, cv::Rect(0, 0, half_width, half_height));
    cv::Mat q1(out, cv::Rect(half_width, 0, half_width, half_height));
    cv::Mat q2(out, cv::Rect(0, half_height, half_width, half_height));
    cv::Mat q3(out, cv::Rect(half_width, half_height, half_width, half_height));
...
void create_inverse_fourier_image_from_complex(const cv::Mat &in,
                                               const ::cv::Mat &origin,
                                               cv::Mat &out) {
    using namespace cv;
...
int main(int argc, char *argv[]) {
    ...
    cv::imwrite(string(argv[1]) + ".idft.jpg", idft_image * 255);
    **/
